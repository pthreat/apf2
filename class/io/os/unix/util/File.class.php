<?php
	...
	namespace apf\io\os\unix\util{
		...
		use apf\io\common\exception\file\CantCreate		as	CantCreateException;
		...
		class File extends CommonFileUtil{
			...
			public static function create($file,$parameters=NULL){

				$file	=	VarUtil::printVar($file,$parameters);

				if(file_exists($file)){

					throw new CantCreateException("Can not create file, file \"$file\" exists");
				...
				if(!is_dir($dir)){
					...
					DirectoryUtil::create($dir,$parameters);

				}

				if(!touch($file)){

					throw new CantCreateException("Can not create file \"$file\"");

				}

				$perms	=	VarUtil::printVar($parameters->find('mode','rw-------')->valueOf());
				self::chmod($file,['perms'=>$perms]);
