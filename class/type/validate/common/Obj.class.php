	...
	namespace apf\type\validate\common{
		...
		class Obj{
			...
			public static function isTraversable($val){
				...
				return $val instanceof \Traversable;

				//Falta poner el metodo toArray si no es instancia de traversable, 
				//hay que chequear por ese metodo
				//Preguntas: Que devuelve $val=new stdclass(); instanceof \Traversable???

			}

			public static function isPrintable($obj,$parameters=NULL){
				...
				$print				=	$parameters->find('print');

				$hasToString		=	self::hasMethod($obj,'toString');
				$hasMagicToString	=	self::hasMethod($obj,'__toString');

				if($print&&$hasToString){

					//Check that the returned value by the toString method is actually a string
					//Could also be an object, consider making a printable type or interface?

					return "{$obj->toString()}";

				}

				if($print&&$hasMagicToString){

					return "$obj";

				}

				return $hasToString||$hasMagicToString;
